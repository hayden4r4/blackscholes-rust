use crate::{constants::*, Greeks, OptionType, Pricing, valuators::black_76::{distributions::*, Inputs}};
...
impl Greeks<f64> for Inputs {
    ...
    fn calc_delta(&self) -> Result<f64, String> {
        ...
        let delta: f64 = match self.option_type {
            OptionType::Call => nd1 * (-self.r * self.t).exp(),
            OptionType::Put => -nd1 * (-self.r * self.t).exp(),
        };
        Ok(delta)
    }
    ...
    fn calc_gamma(&self) -> Result<f64, String> {
        ...
        let gamma: f64 = (-self.r * self.t).exp() * nprimed1 / (self.f * sigma * self.t.sqrt());
        Ok(gamma)
    }
    ...
    fn calc_theta(&self) -> Result<f64, String> {
        ...
        let theta: f64 = match self.option_type {
            OptionType::Call => {
                (-(self.f * sigma * (-self.r * self.t).exp() * nprimed1 / (2.0 * self.t.sqrt()))
                    - self.r * self.k * (-self.r * self.t).exp() * nd2
                    + self.r * self.f * (-self.r * self.t).exp() * nd1)
                    / DAYS_PER_YEAR
            }
            OptionType::Put => {
                (-(self.f * sigma * (-self.r * self.t).exp() * nprimed1 / (2.0 * self.t.sqrt()))
    ...
    fn calc_vega(&self) -> Result<f64, String> {
        let nprimed1: f64 = calc_nprimed1(&self)?;
        let vega: f64 = 0.01 * self.f * (-self.r * self.t).exp() * self.t.sqrt() * nprimed1;
        Ok(vega)
    }
    ...
    fn calc_rho(&self) -> Result<f64, String> {
        ...
        let rho: f64 = match &self.option_type {
            OptionType::Call => {
                -1.0 / 100.0 * self.r * (-self.r * self.t).exp() * (self.f * nd1 - self.k * nd2)
            }
            OptionType::Put => {
                -1.0 / 100.0 * self.r * (-self.r * self.t).exp() * (self.k * nd2 - self.f * nd1)
            }
        };
        Ok(rho)
    }
    ...
    fn calc_epsilon(&self) -> Result<f64, String> {
        ...
        let epsilon: f64 = -self.f * self.t * e_negrt * nd1 * self.option_type;
    ...
    fn calc_vanna(&self) -> Result<f64, String> {
        ...
        let vanna: f64 = d2 * (-self.r * self.t).exp() * nprimed1 * -0.01 / sigma;
        Ok(vanna)
    }
