...
fn bench_greeks(c: &mut Criterion) {
    ...
    for (name, maturity) in configurations.iter() {
        ...
        // Delta
        group.bench_with_input(BenchmarkId::new("delta", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| black_box(inputs.calc_delta().unwrap()))
        });
        
        // Gamma
        group.bench_with_input(BenchmarkId::new("gamma", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| black_box(inputs.calc_gamma().unwrap()))
        });
        
        // Theta
        group.bench_with_input(BenchmarkId::new("theta", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| black_box(inputs.calc_theta().unwrap()))
        });
        
        // Vega
        group.bench_with_input(BenchmarkId::new("vega", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| black_box(inputs.calc_vega().unwrap()))
        });
        
        // Rho
        group.bench_with_input(BenchmarkId::new("rho", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| black_box(inputs.calc_rho().unwrap()))
        });
    }
...
fn bench_all_greeks(c: &mut Criterion) {
    ...
    for (name, inputs) in [("call", call_atm), ("put", put_atm)] {
        group.bench_with_input(BenchmarkId::new("individual", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("all_greeks", name), &inputs, |b, inputs: &Inputs| {
            b.iter(|| {
