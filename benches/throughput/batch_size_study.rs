...
fn bench_batch_size_scaling(c: &mut Criterion) {
    ...
    for size in batch_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("standard_price", size), &inputs, |b, inputs| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("rational_price", size), &inputs, |b, inputs| {
            b.iter(|| {
                let mut results = Vec::with_capacity(inputs.len());
                for input in black_box(inputs) {
        ...
        group.bench_with_input(BenchmarkId::new("all_greeks", size), &inputs, |b, inputs| {
            b.iter(|| {
                let mut results = Vec::with_capacity(inputs.len());
                for input in black_box(inputs) {
