...
#[path = "../common.rs"]
mod common;
use common::{generate_random_inputs, get_sample_config, BatchSize};
...
fn bench_scaling(c: &mut Criterion) {
    ...
    for (idx, &size) in BATCH_SIZES.iter().enumerate() {
        ...
        if let Some(estimate) = group.benchmark_id(&id_str) {
            ...
            println!(
                "Batch size: {}, Time per option: {:.2} ns, Throughput: {:.2} million ops/sec",
                size,
                ns_per_option,
...
fn save_results_to_csv(results: &[ScalingResult]) {
    ...
    if let Err(e) = writeln!(
        file,
        "batch_size,total_time_ns,time_per_option_ns,throughput_ops_per_sec"
    ) {
